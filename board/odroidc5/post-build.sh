...
# 잔여 profile.d 스크립트 제거 (환경변수 통합 작업으로 retropangui.sh 하나로 대체됨)
# target/은 증분 빌드라 rootfs-overlay에서 지운 파일도 이전 빌드 산출물이 남아있을 수 있어
# 명시적으로 삭제해야 함 (libvulkan 강제 복원과 동일한 패턴)
...
mkdir -p "$(dirname "${PRIORITIES_DST}")"
python3 - "${BOARD_DIR}/systems.json" "${PRIORITIES_DST}" << 'PYEOF'
import json, sys
systems_file, out_file = sys.argv[1], sys.argv[2]
with open(systems_file) as f:
    systems = json.load(f)
lines = ["# module_id:system_name:priority (generated by post-build.sh)"]
for s in systems:
    for c in s['cores']:
        lines.append(f"{c['module_id']}:{s['name']}:{c['priority']}")
with open(out_file, 'w') as f:
    f.write('\n'.join(lines) + '\n')
print(f">>> priorities.conf 완료: {out_file}")
PYEOF
