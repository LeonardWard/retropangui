...
#
# 2026-07-10 확인된 한계: -k는 dd로 이미지 파일을 그대로 쓰는데, 이 이미지
# 자체의 파티션 테이블(genimage 산출물)은 boot/overlay 2개만 정의하고 있음 -
# 카드에 이미 3번 파티션(share, exFAT)이 있던 상태로 -k 플래싱하면, dd가
# 새 파티션 테이블(2개짜리)로 MBR을 덮어써서 3번 파티션의 "테이블 항목"이
# 사라짐(실제 바이트는 이미지 크기 이후라 안 건드려짐 - sfdisk로 원래
# 시작 섹터에 파티션을 재선언하면 살릴 수는 있음, 단 정확한 원래 시작
# 섹터를 알아야 함). 조치 안 하면 S61share가 "파티션 없음 = 최초 부팅"으로
# 판단해서 새 exFAT로 재포맷 - 즉 -k도 사실상 전체 초기화와 결과가 같아짐.
# 진짜로 share를 보존하려면 이 스크립트가 기존 파티션 3개를 먼저 확인하고
# dd 대신 boot/overlay 파티션에만 부분적으로 쓰는 방식으로 바뀌어야 함(TODO).
